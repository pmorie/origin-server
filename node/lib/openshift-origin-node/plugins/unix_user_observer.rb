...
module OpenShift
  class UnixUserObserver
    ...
    def after_unix_user_create(user)
      out,err,rc = Utils.oo_spawn("service cgconfig status > /dev/null 2>&1")
      if rc == 0
        out,err,rc = Utils.oo_spawn("/usr/bin/oo-admin-ctl-cgroups startuser #{user.name} > /dev/null")
    ...
    def before_unix_user_destroy(user)
      ...
      out,err,rc = Utils.oo_spawn("service cgconfig status > /dev/null")
      if rc == 0
        Utils.oo_spawn("/usr/bin/oo-admin-ctl-cgroups freezeuser #{user.name} > /dev/null") if rc == 0
      end

      last_access_dir = OpenShift::Config.instance.get("LAST_ACCESS_DIR")
      Utils.oo_spawn("rm -f #{last_access_dir}/#{user.name} > /dev/null")
    end
    ...
    def after_unix_user_destroy(user)
      out,err,rc = Utils.oo_spawn("service cgconfig status > /dev/null")
      Utils.oo_spawn("/usr/bin/oo-admin-ctl-cgroups thawuser #{user.name} > /dev/null") if rc == 0
      Utils.oo_spawn("/usr/bin/oo-admin-ctl-cgroups stopuser #{user.name} > /dev/null") if rc == 0
